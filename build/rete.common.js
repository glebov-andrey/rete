...
function (_Context) {
  ...
  _createClass(NodeEditor, [{
    ...
    value: function selectNode(node) {
      ...
      if (!accumulate) {
        var prevSelected = this.selected.list; // Must clear the selection before triggering 'nodedeselected' so that the handlers observe the deselection.

        this.selected.clear();
        prevSelected.forEach(function (prevNode) {
          return _this3.trigger('nodedeselected', prevNode);
        });
      }
    ...
    key: "toggleNodeSelection",
    value: function toggleNodeSelection(node) {
      if (this.nodes.indexOf(node) === -1) {
        throw new Error('The node does not belong to this editor');
      }
                    ...
                    return regeneratorRuntime.wrap(function _callee$(_context) {
                      while (1) {
                        switch (_context.prev = _context.next) {
                          case 0:
                            node = json.nodes[id];
                            component = _this5.getComponent(node.name);
                            _context.next = 4;
                  ...
                  Object.keys(jsonNode.outputs).forEach(function (key) {
                    ...
                    outputJson.connections.forEach(function (jsonConnection) {
                      ...
                      if (!targetOutput || !targetInput) {
                        return _this5.trigger('error', "IO not found for node ".concat(node.id));
                      }

                      _this5.connect(targetOutput, targetInput, data);
                    });
                  });
                });
                _context2.next = 13;
